...
2. **Key Exchange (RSA + AES):** Alice generates a fast AES key, encrypts it with Bob's public RSA key, and sends it to him over the "internet".
3. **Secure Messaging & Digital Signatures:** Bob encrypts a secret message using the shared AES key, signs it with his private RSA key, and sends it back to Alice.
