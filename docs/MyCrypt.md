...
## What is it for?
Symmetric cryptography uses the **exact same key** (the "Shared Secret") to both encrypt and decrypt data. 
- **The Shared Secret:** This is a single 256-bit AES key that both the sender and receiver must possess. It is "shared" because both parties have it, and "secret" because if anyone else discovers it, they can read the messages and forge signatures.
- **Performance:** It is significantly faster than asymmetric cryptography (RSA), making it ideal for large data.
- **Key Exchange:** Because both parties need the same key, it is usually combined with RSA to securely exchange the AES key first.
...
- **Signature Algorithm (HMAC):** `HmacSHA256`. 
  - **Why a Secret is Required:** Unlike a simple hash (which anyone can calculate), an **HMAC (Hash-based Message Authentication Code)** uses the shared secret key to create the signature. 
  - **Authenticity & Integrity:** This proves that the person who created the signature possesses the secret key (**Authenticity**) and that the message hasn't been altered (**Integrity**). 
  - **Hashed Signing:** The signature is generated based on the **SHA-256 hash** of the message.
